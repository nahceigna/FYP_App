import 'package:flutter/material.dart';
import 'package:fl_chart/fl_chart.dart';

List<Color> gradientColors = [
  const Color(0xff23b6e6),
  const Color(0xff02d39a),
];

Widget barChart(String selectedLocation, int selectedDay) {
  int locationIndex = 0;
  switch (selectedLocation) {
    case "Overall":
      locationIndex = 0;
      break;
    case "1/F":
      locationIndex = 1;
      break;
    case "G/F":
      locationIndex = 2;
      break;
    case "LG1":
      locationIndex = 3;
      break;
    case "LG3":
      break;
    case "LG4":
      locationIndex = 4;
      break;
    default:
      locationIndex = 0;
  }
  return SizedBox(
    height: 215.0,
    child: Card(
      // shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(0)),
      // color: Colors.white,
      child: Stack(
        children: <Widget>[
          Padding(
            padding: const EdgeInsets.fromLTRB(1, 5, 5, 15),
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.stretch,
              mainAxisAlignment: MainAxisAlignment.start,
              mainAxisSize: MainAxisSize.max,
              children: <Widget>[
                Expanded(
                  child: Padding(
                    padding: const EdgeInsets.symmetric(horizontal: 8.0),
                    // child: BarChart(barData()),
                    child: LineChart(lineData(locationIndex, selectedDay)),
                  ),
                ),
              ],
            ),
          ),
        ],
      ),
    ),
  );
}

LineChartData lineData(int location, int day) {
  return LineChartData(
    gridData: FlGridData(
      show: true,
      drawVerticalLine: false,
      horizontalInterval: 200,
      getDrawingHorizontalLine: (value) {
        return FlLine(
          color: const Color(0xFFE9ECEF),
          strokeWidth: 1,
        );
      },
    ),
    lineTouchData: LineTouchData(
        touchTooltipData: LineTouchTooltipData(
      getTooltipItems: (value) {
        return value
            .map((e) => LineTooltipItem("People: ${(e.y).toStringAsFixed(0)}",
                const TextStyle(fontSize: 12.0, color: Colors.white)))
            .toList();
      },
      tooltipBgColor: Colors.black,
    )),
    titlesData: FlTitlesData(
      show: true,
      rightTitles: SideTitles(showTitles: false),
      topTitles: SideTitles(showTitles: false),
      bottomTitles: SideTitles(
        showTitles: true,
        reservedSize: 10,
        interval: 1,
        getTextStyles: (context, value) =>
            const TextStyle(color: Color(0xFF6C757D), fontSize: 12),
        getTitles: (value) {
          switch (value.toInt()) {
            case 2:
              return '2am';
            case 6:
              return '6am';
            case 10:
              return '10am';
            case 14:
              return '2pm';
            case 18:
              return '6pm';
            case 22:
              return '10pm';
          }
          return '';
        },
        margin: 5,
      ),
      leftTitles: SideTitles(
        showTitles: true,
        interval: 1,
        getTextStyles: (context, value) => const TextStyle(
          color: Color(0xFF6C757D),
          fontSize: 12,
        ),
        getTitles: (value) {
          switch (value.toInt()) {
            case 199:
              return '200';
            case 399:
              return '400';
            case 599:
              return '600';
            case 799:
              return '800';
          }
          return '';
        },
        reservedSize: 20,
        margin: 10,
      ),
    ),
    borderData: FlBorderData(show: false),
    minX: 0,
    maxX: 23,
    minY: 0,
    maxY: 1000,
    lineBarsData: [
      LineChartBarData(
        spots: dataList[location][day],
        isCurved: true,
        colors: gradientColors,
        barWidth: 3,
        isStrokeCapRound: true,
        dotData: FlDotData(show: false),
        belowBarData: BarAreaData(
          show: true,
          colors:
              gradientColors.map((color) => color.withOpacity(0.3)).toList(),
        ),
      ),
    ],
  );
}

const List<List<List<FlSpot>>> dataList = [
  [
    [
      FlSpot(0, 100),
      FlSpot(2, 70),
      FlSpot(4, 200),
      FlSpot(6, 500),
      FlSpot(8, 600),
      FlSpot(10, 650),
      FlSpot(12, 700),
      FlSpot(14, 800),
      FlSpot(16, 750),
      FlSpot(18, 700),
      FlSpot(20, 400),
      FlSpot(22, 200),
      FlSpot(23, 150),
    ],
    [
      FlSpot(0, 100),
      FlSpot(2, 70),
      FlSpot(4, 200),
      FlSpot(6, 400),
      FlSpot(8, 500),
      FlSpot(10, 550),
      FlSpot(12, 700),
      FlSpot(14, 860),
      FlSpot(16, 700),
      FlSpot(18, 600),
      FlSpot(20, 400),
      FlSpot(22, 200),
      FlSpot(23, 150),
    ],
    [
      FlSpot(0, 70),
      FlSpot(2, 100),
      FlSpot(4, 200),
      FlSpot(6, 400),
      FlSpot(8, 300),
      FlSpot(10, 700),
      FlSpot(12, 600),
      FlSpot(14, 850),
      FlSpot(16, 800),
      FlSpot(18, 750),
      FlSpot(20, 500),
      FlSpot(22, 100),
      FlSpot(23, 150),
    ],
    [
      FlSpot(0, 70),
      FlSpot(2, 100),
      FlSpot(4, 200),
      FlSpot(6, 400),
      FlSpot(8, 500),
      FlSpot(10, 700),
      FlSpot(12, 700),
      FlSpot(14, 850),
      FlSpot(16, 800),
      FlSpot(18, 750),
      FlSpot(20, 300),
      FlSpot(22, 100),
      FlSpot(23, 150),
    ],
    [
      FlSpot(0, 70),
      FlSpot(2, 100),
      FlSpot(4, 250),
      FlSpot(6, 440),
      FlSpot(8, 300),
      FlSpot(10, 700),
      FlSpot(12, 600),
      FlSpot(14, 750),
      FlSpot(16, 800),
      FlSpot(18, 750),
      FlSpot(20, 400),
      FlSpot(22, 100),
      FlSpot(23, 150),
    ],
    [
      FlSpot(0, 70),
      FlSpot(2, 100),
      FlSpot(4, 200),
      FlSpot(6, 400),
      FlSpot(8, 300),
      FlSpot(10, 300),
      FlSpot(12, 500),
      FlSpot(14, 650),
      FlSpot(16, 300),
      FlSpot(18, 250),
      FlSpot(20, 100),
      FlSpot(22, 100),
      FlSpot(23, 150),
    ],
    [
      FlSpot(0, 70),
      FlSpot(2, 100),
      FlSpot(4, 200),
      FlSpot(6, 400),
      FlSpot(8, 300),
      FlSpot(10, 300),
      FlSpot(12, 200),
      FlSpot(14, 150),
      FlSpot(16, 300),
      FlSpot(18, 250),
      FlSpot(20, 100),
      FlSpot(22, 200),
      FlSpot(23, 50),
    ]
  ],
  [
    [
      FlSpot(0, 70),
      FlSpot(2, 60),
      FlSpot(4, 170),
      FlSpot(6, 478),
      FlSpot(8, 356),
      FlSpot(10, 746),
      FlSpot(12, 617),
      FlSpot(14, 885),
      FlSpot(16, 823),
      FlSpot(18, 757),
      FlSpot(20, 509),
      FlSpot(22, 150),
      FlSpot(23, 150),
    ],
    [
      FlSpot(0, 105),
      FlSpot(2, 76),
      FlSpot(4, 290),
      FlSpot(6, 530),
      FlSpot(8, 640),
      FlSpot(10, 670),
      FlSpot(12, 790),
      FlSpot(14, 834),
      FlSpot(16, 779),
      FlSpot(18, 764),
      FlSpot(20, 457),
      FlSpot(22, 203),
      FlSpot(23, 160),
    ],
    [
      FlSpot(0, 130),
      FlSpot(2, 70),
      FlSpot(4, 260),
      FlSpot(6, 480),
      FlSpot(8, 529),
      FlSpot(10, 548),
      FlSpot(12, 742),
      FlSpot(14, 884),
      FlSpot(16, 736),
      FlSpot(18, 682),
      FlSpot(20, 471),
      FlSpot(22, 237),
      FlSpot(23, 138),
    ],
    [
      FlSpot(0, 62),
      FlSpot(2, 180),
      FlSpot(4, 292),
      FlSpot(6, 472),
      FlSpot(8, 548),
      FlSpot(10, 748),
      FlSpot(12, 377),
      FlSpot(14, 840),
      FlSpot(16, 883),
      FlSpot(18, 755),
      FlSpot(20, 383),
      FlSpot(22, 138),
      FlSpot(23, 152),
    ],
    [
      FlSpot(0, 72),
      FlSpot(2, 180),
      FlSpot(4, 291),
      FlSpot(6, 464),
      FlSpot(8, 378),
      FlSpot(10, 737),
      FlSpot(12, 638),
      FlSpot(14, 737),
      FlSpot(16, 882),
      FlSpot(18, 759),
      FlSpot(20, 426),
      FlSpot(22, 182),
      FlSpot(23, 128),
    ],
    [
      FlSpot(0, 70),
      FlSpot(2, 100),
      FlSpot(4, 200),
      FlSpot(6, 400),
      FlSpot(8, 300),
      FlSpot(10, 300),
      FlSpot(12, 500),
      FlSpot(14, 650),
      FlSpot(16, 300),
      FlSpot(18, 250),
      FlSpot(20, 100),
      FlSpot(22, 100),
      FlSpot(23, 150),
    ],
    [
      FlSpot(0, 70),
      FlSpot(2, 100),
      FlSpot(4, 200),
      FlSpot(6, 400),
      FlSpot(8, 300),
      FlSpot(10, 300),
      FlSpot(12, 200),
      FlSpot(14, 150),
      FlSpot(16, 300),
      FlSpot(18, 250),
      FlSpot(20, 100),
      FlSpot(22, 200),
      FlSpot(23, 50),
    ]
  ],
  [
    [
      FlSpot(0, 100),
      FlSpot(2, 70),
      FlSpot(4, 200),
      FlSpot(6, 400),
      FlSpot(8, 500),
      FlSpot(10, 550),
      FlSpot(12, 700),
      FlSpot(14, 860),
      FlSpot(16, 700),
      FlSpot(18, 600),
      FlSpot(20, 400),
      FlSpot(22, 200),
      FlSpot(23, 150),
    ],
    [
      FlSpot(0, 100),
      FlSpot(2, 70),
      FlSpot(4, 200),
      FlSpot(6, 500),
      FlSpot(8, 600),
      FlSpot(10, 650),
      FlSpot(12, 700),
      FlSpot(14, 800),
      FlSpot(16, 750),
      FlSpot(18, 700),
      FlSpot(20, 400),
      FlSpot(22, 200),
      FlSpot(23, 150),
    ],
    [
      FlSpot(0, 70),
      FlSpot(2, 100),
      FlSpot(4, 200),
      FlSpot(6, 400),
      FlSpot(8, 300),
      FlSpot(10, 700),
      FlSpot(12, 600),
      FlSpot(14, 850),
      FlSpot(16, 800),
      FlSpot(18, 750),
      FlSpot(20, 500),
      FlSpot(22, 100),
      FlSpot(23, 150),
    ],
    [
      FlSpot(0, 70),
      FlSpot(2, 100),
      FlSpot(4, 200),
      FlSpot(6, 400),
      FlSpot(8, 500),
      FlSpot(10, 700),
      FlSpot(12, 700),
      FlSpot(14, 850),
      FlSpot(16, 800),
      FlSpot(18, 750),
      FlSpot(20, 300),
      FlSpot(22, 100),
      FlSpot(23, 150),
    ],
    [
      FlSpot(0, 70),
      FlSpot(2, 100),
      FlSpot(4, 250),
      FlSpot(6, 440),
      FlSpot(8, 300),
      FlSpot(10, 700),
      FlSpot(12, 600),
      FlSpot(14, 750),
      FlSpot(16, 800),
      FlSpot(18, 750),
      FlSpot(20, 400),
      FlSpot(22, 100),
      FlSpot(23, 150),
    ],
    [
      FlSpot(0, 70),
      FlSpot(2, 100),
      FlSpot(4, 200),
      FlSpot(6, 400),
      FlSpot(8, 300),
      FlSpot(10, 300),
      FlSpot(12, 500),
      FlSpot(14, 650),
      FlSpot(16, 300),
      FlSpot(18, 250),
      FlSpot(20, 100),
      FlSpot(22, 100),
      FlSpot(23, 150),
    ],
    [
      FlSpot(0, 70),
      FlSpot(2, 100),
      FlSpot(4, 200),
      FlSpot(6, 400),
      FlSpot(8, 300),
      FlSpot(10, 300),
      FlSpot(12, 200),
      FlSpot(14, 150),
      FlSpot(16, 300),
      FlSpot(18, 250),
      FlSpot(20, 100),
      FlSpot(22, 200),
      FlSpot(23, 50),
    ]
  ],
  [
    [
      FlSpot(0, 70),
      FlSpot(2, 100),
      FlSpot(4, 250),
      FlSpot(6, 440),
      FlSpot(8, 300),
      FlSpot(10, 700),
      FlSpot(12, 600),
      FlSpot(14, 750),
      FlSpot(16, 800),
      FlSpot(18, 750),
      FlSpot(20, 400),
      FlSpot(22, 100),
      FlSpot(23, 150),
    ],
    [
      FlSpot(0, 100),
      FlSpot(2, 70),
      FlSpot(4, 200),
      FlSpot(6, 500),
      FlSpot(8, 600),
      FlSpot(10, 650),
      FlSpot(12, 700),
      FlSpot(14, 800),
      FlSpot(16, 750),
      FlSpot(18, 700),
      FlSpot(20, 400),
      FlSpot(22, 200),
      FlSpot(23, 150),
    ],
    [
      FlSpot(0, 100),
      FlSpot(2, 70),
      FlSpot(4, 200),
      FlSpot(6, 400),
      FlSpot(8, 500),
      FlSpot(10, 550),
      FlSpot(12, 700),
      FlSpot(14, 860),
      FlSpot(16, 700),
      FlSpot(18, 600),
      FlSpot(20, 400),
      FlSpot(22, 200),
      FlSpot(23, 150),
    ],
    [
      FlSpot(0, 70),
      FlSpot(2, 100),
      FlSpot(4, 200),
      FlSpot(6, 400),
      FlSpot(8, 300),
      FlSpot(10, 700),
      FlSpot(12, 600),
      FlSpot(14, 850),
      FlSpot(16, 800),
      FlSpot(18, 750),
      FlSpot(20, 500),
      FlSpot(22, 100),
      FlSpot(23, 150),
    ],
    [
      FlSpot(0, 70),
      FlSpot(2, 100),
      FlSpot(4, 200),
      FlSpot(6, 400),
      FlSpot(8, 500),
      FlSpot(10, 700),
      FlSpot(12, 700),
      FlSpot(14, 850),
      FlSpot(16, 800),
      FlSpot(18, 750),
      FlSpot(20, 300),
      FlSpot(22, 100),
      FlSpot(23, 150),
    ],
    [
      FlSpot(0, 70),
      FlSpot(2, 100),
      FlSpot(4, 200),
      FlSpot(6, 400),
      FlSpot(8, 300),
      FlSpot(10, 300),
      FlSpot(12, 500),
      FlSpot(14, 650),
      FlSpot(16, 300),
      FlSpot(18, 250),
      FlSpot(20, 100),
      FlSpot(22, 100),
      FlSpot(23, 150),
    ],
    [
      FlSpot(0, 70),
      FlSpot(2, 100),
      FlSpot(4, 200),
      FlSpot(6, 400),
      FlSpot(8, 300),
      FlSpot(10, 300),
      FlSpot(12, 200),
      FlSpot(14, 150),
      FlSpot(16, 300),
      FlSpot(18, 250),
      FlSpot(20, 100),
      FlSpot(22, 200),
      FlSpot(23, 50),
    ]
  ],
  [
    [
      FlSpot(0, 70),
      FlSpot(2, 100),
      FlSpot(4, 200),
      FlSpot(6, 400),
      FlSpot(8, 300),
      FlSpot(10, 300),
      FlSpot(12, 200),
      FlSpot(14, 150),
      FlSpot(16, 300),
      FlSpot(18, 250),
      FlSpot(20, 100),
      FlSpot(22, 200),
      FlSpot(23, 50),
    ],
    [
      FlSpot(0, 100),
      FlSpot(2, 70),
      FlSpot(4, 200),
      FlSpot(6, 500),
      FlSpot(8, 600),
      FlSpot(10, 650),
      FlSpot(12, 700),
      FlSpot(14, 800),
      FlSpot(16, 750),
      FlSpot(18, 700),
      FlSpot(20, 400),
      FlSpot(22, 200),
      FlSpot(23, 150),
    ],
    [
      FlSpot(0, 100),
      FlSpot(2, 70),
      FlSpot(4, 200),
      FlSpot(6, 400),
      FlSpot(8, 500),
      FlSpot(10, 550),
      FlSpot(12, 700),
      FlSpot(14, 860),
      FlSpot(16, 700),
      FlSpot(18, 600),
      FlSpot(20, 400),
      FlSpot(22, 200),
      FlSpot(23, 150),
    ],
    [
      FlSpot(0, 70),
      FlSpot(2, 100),
      FlSpot(4, 200),
      FlSpot(6, 400),
      FlSpot(8, 300),
      FlSpot(10, 700),
      FlSpot(12, 600),
      FlSpot(14, 850),
      FlSpot(16, 800),
      FlSpot(18, 750),
      FlSpot(20, 500),
      FlSpot(22, 100),
      FlSpot(23, 150),
    ],
    [
      FlSpot(0, 70),
      FlSpot(2, 100),
      FlSpot(4, 200),
      FlSpot(6, 400),
      FlSpot(8, 500),
      FlSpot(10, 700),
      FlSpot(12, 700),
      FlSpot(14, 850),
      FlSpot(16, 800),
      FlSpot(18, 750),
      FlSpot(20, 300),
      FlSpot(22, 100),
      FlSpot(23, 150),
    ],
    [
      FlSpot(0, 70),
      FlSpot(2, 100),
      FlSpot(4, 250),
      FlSpot(6, 440),
      FlSpot(8, 300),
      FlSpot(10, 700),
      FlSpot(12, 600),
      FlSpot(14, 750),
      FlSpot(16, 800),
      FlSpot(18, 750),
      FlSpot(20, 400),
      FlSpot(22, 100),
      FlSpot(23, 150),
    ],
    [
      FlSpot(0, 70),
      FlSpot(2, 100),
      FlSpot(4, 200),
      FlSpot(6, 400),
      FlSpot(8, 300),
      FlSpot(10, 300),
      FlSpot(12, 500),
      FlSpot(14, 650),
      FlSpot(16, 300),
      FlSpot(18, 250),
      FlSpot(20, 100),
      FlSpot(22, 100),
      FlSpot(23, 150),
    ]
  ],
  [
    [
      FlSpot(0, 70),
      FlSpot(2, 100),
      FlSpot(4, 200),
      FlSpot(6, 400),
      FlSpot(8, 500),
      FlSpot(10, 700),
      FlSpot(12, 700),
      FlSpot(14, 850),
      FlSpot(16, 800),
      FlSpot(18, 750),
      FlSpot(20, 300),
      FlSpot(22, 100),
      FlSpot(23, 150),
    ],
    [
      FlSpot(0, 100),
      FlSpot(2, 70),
      FlSpot(4, 200),
      FlSpot(6, 500),
      FlSpot(8, 600),
      FlSpot(10, 650),
      FlSpot(12, 700),
      FlSpot(14, 800),
      FlSpot(16, 750),
      FlSpot(18, 700),
      FlSpot(20, 400),
      FlSpot(22, 200),
      FlSpot(23, 150),
    ],
    [
      FlSpot(0, 100),
      FlSpot(2, 70),
      FlSpot(4, 200),
      FlSpot(6, 400),
      FlSpot(8, 500),
      FlSpot(10, 550),
      FlSpot(12, 700),
      FlSpot(14, 860),
      FlSpot(16, 700),
      FlSpot(18, 600),
      FlSpot(20, 400),
      FlSpot(22, 200),
      FlSpot(23, 150),
    ],
    [
      FlSpot(0, 70),
      FlSpot(2, 100),
      FlSpot(4, 200),
      FlSpot(6, 400),
      FlSpot(8, 300),
      FlSpot(10, 700),
      FlSpot(12, 600),
      FlSpot(14, 850),
      FlSpot(16, 800),
      FlSpot(18, 750),
      FlSpot(20, 500),
      FlSpot(22, 100),
      FlSpot(23, 150),
    ],
    [
      FlSpot(0, 70),
      FlSpot(2, 100),
      FlSpot(4, 250),
      FlSpot(6, 440),
      FlSpot(8, 300),
      FlSpot(10, 700),
      FlSpot(12, 600),
      FlSpot(14, 750),
      FlSpot(16, 800),
      FlSpot(18, 750),
      FlSpot(20, 400),
      FlSpot(22, 100),
      FlSpot(23, 150),
    ],
    [
      FlSpot(0, 70),
      FlSpot(2, 100),
      FlSpot(4, 200),
      FlSpot(6, 400),
      FlSpot(8, 300),
      FlSpot(10, 300),
      FlSpot(12, 500),
      FlSpot(14, 650),
      FlSpot(16, 300),
      FlSpot(18, 250),
      FlSpot(20, 100),
      FlSpot(22, 100),
      FlSpot(23, 150),
    ],
    [
      FlSpot(0, 70),
      FlSpot(2, 100),
      FlSpot(4, 200),
      FlSpot(6, 400),
      FlSpot(8, 300),
      FlSpot(10, 300),
      FlSpot(12, 200),
      FlSpot(14, 150),
      FlSpot(16, 300),
      FlSpot(18, 250),
      FlSpot(20, 100),
      FlSpot(22, 200),
      FlSpot(23, 50),
    ]
  ],
];
